...
$current_category_id = ($current_category && ! is_wp_error($current_category) && isset($current_category->term_id))
...
<div id="main">
    ...
    $currentMin = isset($_GET['min_price']) ? (int)$_GET['min_price'] : 300;  // デフォルト最小価格
    $currentMax = isset($_GET['max_price']) ? (int)$_GET['max_price'] : 4000;  // デフォルト最大価格

    // 現在表示中のカテゴリーの投稿を取得するクエリ
    $land_query = new WP_Query(array(
        'cat' => $current_category_id,  // 現在のカテゴリID
...
if ($land_query->found_posts > 10): ?>
    <div class="blog-pagination">
        ...
        echo paginate_links(array(
            ...
            'total' => $land_query->max_num_pages,
            'prev_text' => '&#8592;',
            'next_text' => '&#8594;',
        ));
        ?>
