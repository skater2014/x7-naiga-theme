 ...
 *     「ChatGPTモーダル表示設定」
 *
 * を作っているのが、このテーマ内のPHPです。
 *
 *
 * ■ プラグインなのか？
 *
 * これは独立したWordPressプラグインではありません。
 *
 * 通常のプラグインなら、
 *
 *     wp-content/plugins/
 ...
 * の中に専用のプラグインがあります。
 *
 * 今回のChatGPTモーダルはそうではなく、
 *
 *     wp-content/themes/x7-naigaicorp-20260713/
 *
 * の中にChatGPT機能を作っています。
 *
 * つまり、
 ...
 * ■ なぜ管理画面にメニューが表示されるのか
 *
 * WordPressはプラグインを作らなくても、
 * テーマのPHPから
 *
 *     add_menu_page()
 *
 * を使うことで、
 * WordPress管理画面に独自メニューを追加できます。
 *
 * この仕組みを使って、
 *
 *     ChatGPTモーダル
 ...
 * ■ 補足
 *
 * 管理画面にメニューが存在するからといって、
 * 必ずしもWordPressプラグインとは限りません。
 *
 * 今回は、
 *
 *     テーマ
...
add_action('wp_enqueue_scripts', function () {
    ...
    wp_localize_script('naigai-ai-chat', 'naigaiAiChatAjax', array(
        ...
        /*
             * ============================================================
             * 2026-08-02 ChatGPTモーダル不具合 修正内容
             * ============================================================
             *
             * ■ 修正前に起きていた問題
             *
             * ChatGPTモーダル表示設定で、
             * フロントページ以外のページにもチェックを入れると、
             *
             *     モーダル自体は表示される
             *
             * ところまでは正常でした。
             ...
             * ■ 原因
             *
             * WordPressからChatGPTへ通信するときには
             * 「nonce」というセキュリティ確認用の値を使用します。
             *
             * 簡単に言えば、
             *
             *     「この通信は正しいサイトから送られたものです」
             *
             * と確認するための合言葉のようなものです。
             *
             *
             * 修正前、このファイルでは
             *
             *     naigai_ai_chat_nonce
             *
             * という名前のnonceを作っていました。
             *
             * ところが、質問を受け取るPHP側では
             *
             *     openai-chatmodal_nonce
             *
             * という別の名前を確認していました。
             *
             *
             ...
             *
             * 名前が一致していないため、
             * WordPressが通信を正常なものとして認証できず、
             * ChatGPTへの質問処理が失敗していました。
             *
             *
             * ■ 今回どこを修正したのか
             *
             * 修正前：
             *
             *     wp_create_nonce('naigai_ai_chat_nonce')
             *
             * 修正後：
             *
             *     wp_create_nonce('openai-chatmodal_nonce')
             *
             * に変更しました。
             *
             *
             ...
             *
             *
             * ■ なぜフロントページだけは以前から動いていたのか
             *
             * フロントページでは、
             * 以前から存在していた別の読み込み処理によって
             * ChatGPT通信に必要な正しいnonceが渡されていました。
             *
             * そのため、
             *
             *     フロントページだけ動く
