...
for ($i = 1; $i <= 20; $i++) {
  ...
  $hero_slides[] = array(
    'image_id' => $image_id,
    'alt'      => $get_attachment_alt($image_id, $page_title),
  );
}
...
if (!empty($gallery_images)) {
  foreach ($gallery_images as $index => $image_id) {
    ...
    $label    = isset($room_gallery_labels[$index]) ? $normalize_label($room_gallery_labels[$index]) : '';
    $fallback = $label !== '' ? $label : $page_title;
...
<style>
  ...
  /* =========================================================
   * 1. 共通テキスト・見出し
   * - 各 section の見出しブロックを共通化
   * - PCは左寄せを基本
   * - 下部 gallery だけ .section-heading--center で中央寄せ
   * ========================================================= */
  ...
  /* =========================================================
   * 2. 本文エリア内の通常テキスト
   * - the_content() から出る本文pを整える
   * - h2 / h3 は「.content 直下だけ」に限定
   *   → section 内の .section-title まで巻き込まないため
   * ========================================================= */
   ...
   * 5. Media
   * - MP4 / MP3 セクション
   * - section-heading は共通ルールを基本利用
   * - この section では動画本体を全幅で見せる
   * ========================================================= */
   ...
   * 7. Related
   * - PC: 既存 masonry 風グリッド
   * - TB/SP: related-mobile-slider に切替
   * ========================================================= */
   ...
   * - 見出しはここで中央寄せに切替
   * - hero 高さ縮小
   * - related は mobile slider に切替
   * - shell系余白調整
   * ========================================================= */
  @media (max-width: 1024px) {

    /* TB以下では主要セクション見出しを中央寄せ */
    ...
    /* TB以下ではPCグリッド非表示、mobile slider を表示 */
    .room-gallery-page .related-desktop-only {
      display: none;
    }
    ...
    <section class="room-hero">
        ...
        <div class="swiper room-main-swiper<?php echo count($hero_slides) <= 1 ? ' is-static' : ''; ?>">
          ...
          <div class="room-hero-overlay">
            <div class="room-hero-copy">
              ...
              <h1 class="room-hero-title"><?php the_title(); ?></h1>
      ...
      <?php else : ?>
        <div class="room-hero-static">
          ...
          <div class="room-hero-overlay">
            <div class="room-hero-copy">
              <p class="room-hero-eyebrow">NASU ROOM STYLE</p>
              <h1 class="room-hero-title"><?php the_title(); ?></h1>
              <p class="room-hero-lead">東京と那須、二つの魅力を取り入れた暮らしを実現したい方へ。</p>
              <p class="room-hero-text">二拠点生活・移住・別荘購入を検討する方に向けて、理想の住まいと暮らし方をご提案します。</p>
            </div>
          </div>
        </div>
      <?php endif; ?>
