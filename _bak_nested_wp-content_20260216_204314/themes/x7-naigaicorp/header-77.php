...
})(window,document,'script','dataLayer','GTM-P6LLPHR5');</script>
...
.navg_under{
  ...
  overflow:visible;     /* 子のサブメニューが切れないように */
}
.navg__list{
...
.navg__list ul.sub-menu:not(.mega-sub-menu){
  display: none !important;           /* WPが inline で出す display:block を潰す */
  position: absolute; top: 100%; left: 0;
  min-width: 200px; box-sizing: border-box;
  padding: 0;                          /* li にボーダーを付けるのでここは 0 */
  background: #004bba;
  border: 2px solid #fff;              /* ← 白い角なし枠 */
...
.navg__list ul.sub-menu:not(.mega-sub-menu) a{
  ...
  transition: background .2s ease, color .2s ease; /* ← トランジション */
}
...
@media (min-width:1200px){
  ...
  .header-menu-pc{ display:flex !important; width:1200px; margin:0 auto; }
  ...
  .navg__list > .mega-menu-item > .mega-sub-menu{
    ...
    left:50% !important;                 /* 中央基準 */
    transform:translateX(-50%) !important;/* 中央へ */
    right:auto !important;

    width:1180px !important;
    max-width:calc(100vw - 40px);        /* はみ出し保険 */
    box-sizing:border-box;
    background:#004bba; padding:10px;
...
@media (min-width:1200px){
  .mega-sub-menu{
    display: none; /* hover時にflexで出すのは既存のまま */
    align-items: stretch;
    gap: 20px;              /* カラム間の余白 */
  }
  ...
  .blog-post-media{
    width: 100%;
    aspect-ratio: 16 / 9;  /* ←比率固定。高さ固定にしたいなら `height: 180px;` に変更 */
    /* height: 180px; */    /* ←こちらを使う場合は aspect-ratio は消す */
    overflow: hidden;
    background: #000;
    display: flex;
    align-items: center;
  ...
  .blog-post-media iframe{
    width: 100%;
    height: 100%;
    object-fit: cover;     /* 画像トリミングで枠にフィット */
    display: block;
  }
...
@media (max-width:1199px){
  ...
  .navg__list{
    ...
    align-items:stretch; gap:6px;    /* 均等すぎを防ぐ隙間 */
    background:#004bba; overflow:visible;
  }
  .navg__list > li{ flex:0 0 auto; position:relative; } /* ← 伸びない */
  .navg__list > li > a{
  ...
  @media (max-width:1024px){
    .navg__list > li:hover > a,
    .navg__list .sub-menu li:hover > a{ background:transparent !important; }
  }
}
...
@media (max-width: 1199.98px){ /* ←スマホ〜タブレット用のルール */

  /* ① 親ひとつだけ固定（.header-wrapper が“箱ごと”固定される） */
  .header-wrapper{
    position: fixed;                 /* 画面に貼り付く（スクロールしても動かない） */
    top: var(--admin-offset, 0px);   /* WPの黒バー高さぶん下げる（JSが数値を入れる） */
    left: 0; right: 0;               /* 横いっぱいに広げる */
    z-index: 23000;                  /* 手前に出す（メニューが隠れない） */
    background: var(--header-bg);    /* 下が透けないよう背景色を付ける */

    /* ★ポイント：
       flow-root = この要素を“独立した箱（BFC）”にする。
       → 子どもの margin-top が親の外に食い込む（相殺）問題を防げる。 */
    display: flow-root;              /* 子の上マージン食い込み防止（BFCを作る） */
  }
  ...
  .header__top_under,
  .navg_under{
    position: static !important;     /* “普通の配置”に戻す（top/left等は効かない） */
  }
  ...
  ~ :is(.home-view,
    ...
    padding-top: var(--hdr-spacer, 0px) !important; /* ← ここが“押し下げ”の本体 */
    margin-top: 0 !important;                       /* ← マージン相殺のズレ防止 */
  }
  ...
  .page-wrapper > *:first-child{
    margin-top: 0 !important;        /* 先頭子の上マージン食い込みを止める */
  }
}
...
@media (min-width: 768px){
  .navg_under{ position: relative; top: auto; }
}
      ...
      <?php if ($mobilePhoneTest) : ?>
        <div class="phone-container"><!-- /.phone-container START -->
          <a href="tel:<?php echo esc_attr($mobilePhoneTest); ?>" class="icon icon-phone" id="phone-link">
            <svg class="icon icon-phone" width="24" height="24"><use xlink:href="#icon-phone"></use></svg>
