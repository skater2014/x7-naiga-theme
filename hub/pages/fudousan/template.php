 ...
 *   .home-posts
 *   .archive-post-box
 * - 価格・住所・面積などの表示は property-info-table.php を使う
 * - このテンプレート内で物件メタキーを直接整形しない
...
if ($query->have_posts()) {
  while ($query->have_posts()) {
    ...
    $data    = naigai_fudo_get_card_data($post_id);
  ...
  <section class="fudo-hero">
    <div class="fudo-shell fudo-hero__grid">
      <div class="fudo-hero__body">
        <p class="fudo-kicker">Nasu Real Estate</p>
        <h1>那須の土地・住まい・不動産相談</h1>
        <p>
          那須エリアを中心に、土地探し・住宅購入・賃貸・売却相談まで。
          暮らしと事業に合わせた不動産情報をご案内します。
        </p>

        <div class="fudo-hero__actions">
          <a href="#fudousan-list" class="fudo-btn fudo-btn--primary">物件を探す</a>
          <a href="<?php echo esc_url(home_url('/contact/')); ?>" class="fudo-btn fudo-btn--ghost">資料請求・相談</a>
        </div>
      </div>

      <div class="fudo-hero__panel" aria-label="那須エリアの不動産相談">
        <p>土地 / 新築 / 中古住宅 / 賃貸 / 売却相談</p>
        <strong>那須での暮らしを探す入口</strong>
      </div>
    </div>
  </section>
      ...
      <div class="fudo-entry-grid">
        <?php foreach ($entry_cards as $card) : ?>
          <a class="fudo-entry-card fudo-entry-card--<?php echo esc_attr($card['key']); ?>" href="<?php echo esc_url($card['href']); ?>">
            ...
            <span class="fudo-entry-card__arrow">→</span>
          </a>
        <?php endforeach; ?>
      </div>
    </div>
...
<div class="home-posts archive-posts fudo-posts">
        <?php foreach ($items as $item) : ?>
          ...
          if (!empty($data['map_html'])) {
            $map_payload = base64_encode($data['map_html']);
          }
          ?>

          <?php
          $card_post_id = isset($item['post_id']) ? (int) $item['post_id'] : (int) $post_id;
          if ($card_post_id > 0) {
            $post_id = $card_post_id;
          }
              ...
              <div class="fudo-post-card__actions">
                ...
                <?php if (!$is_blog_card) : ?>
                  ...
                  <a
                    ...
                    data-post-id="<?php echo esc_attr($post_id); ?>" data-reservation-title="<?php echo esc_attr(get_the_title($post_id)); ?>">
                    来店予約
                  </a>
                <?php endif; ?>
              </div>
        ...
        <?php if ((int) $counts['fudo-used'] === 0) : ?>
          <article class="archive-post-box main-custom-post fudo-empty-card fudo-used">
            <div class="fudo-empty-card__inner">
              <p class="fudo-empty-card__label">掲載準備中</p>
              <h3>現在、中古住宅の掲載物件はありません。</h2>
        ...
        <?php if ((int) $counts['fudo-rent'] === 0) : ?>
          <article class="archive-post-box main-custom-post fudo-empty-card fudo-rent">
            <div class="fudo-empty-card__inner">
              <p class="fudo-empty-card__label">掲載準備中</p>
              <h3>現在、賃貸物件の掲載物件はありません。</h2>
