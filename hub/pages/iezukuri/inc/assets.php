 ...
 * =========================================================
 *
 * 家づくりフロント側のCSS/JS読み込み担当。
 *
 * 初心者向けに言うと:
 * - 公開ページに必要なCSS/JSを WordPress に登録して読み込ませる係。
 * - /wp-admin/ の編集画面用ではない。
 * - 管理画面用CSS/JSは admin/enqueue.php 側で扱う。
 *
 * 今回の3カード切替での役割:
 * 1. iezukuri-plan-detail-switch.css を読み込む
 * 2. iezukuri-plan-detail-switch.js を読み込む
 * 3. JSより前に window.NAIGAI_IEZ_PLAN_HOME を出力する
 *
 * window.NAIGAI_IEZ_PLAN_HOME とは:
 * - 3カード切替用のデータ台帳。
 * - one-family / two-family / dual-life のデータを持つ。
 * - JSはここから該当データを読んで、赤枠内のHTMLを作る。
 *
 * 紐づくファイル:
 * - inc/plan-home-switch-data.php
 *   → window.NAIGAI_IEZ_PLAN_HOME の中身を作る
 *
 * - js/iezukuri-plan-detail-switch.js
 *   → data-term を読み、window.NAIGAI_IEZ_PLAN_HOME から表示内容を切り替える
 *
 * - css/iezukuri-plan-detail-switch.css
 *   → 赤枠内の .iez-switch-* デザインを担当する
 *
 * 注意:
 * - loader.php 側では同じCSS/JSを読み込まない。
 * - ここを正の読み込み場所にする。
 * =========================================================
 */
...
/**
 * /iezukuri トップLP service-panel.css
 *
 * 役割:
 * - 3カード
 * - 選択パネル stack
 * - term別マーケティング表
 *
 * top.css の @import ではなく、filemtime付きで直読み込みする。
 */
if (!function_exists('naigai_iezukuri_enqueue_service_panel_css')) {
    function naigai_iezukuri_enqueue_service_panel_css()
    {
        if (!is_page('iezukuri')) {
            return;
        }

        $rel = '/hub/pages/iezukuri/css/top/service-panel.css';
        ...
        wp_enqueue_style(
            'naigai-iezukuri-service-panel-direct',
...
add_action('wp_enqueue_scripts', 'naigai_iezukuri_enqueue_service_panel_css', 950);
