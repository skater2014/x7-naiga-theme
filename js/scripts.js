...
function initHomeIsotope($) {
   ...
   *
   * 役割:
   * - /fudousan/ 以外のページで .home-posts / .home-cats-selection を扱う。
   * - .archive-post-box を Isotope の fitRows で並べる。
   *
   * /fudousan/ で止める理由:
   * - /fudousan/ は hub/pages/fudousan/js/fudousan-page.js が専用管理する。
   * - ここでも /fudousan/ の .home-posts を動かすと、Isotope が二重初期化される。
   * - 二重初期化は、タブ切替時の上方向ジャンプ、activeずれ、カード位置の乱れの原因になる。
   *
   * 結論:
   * - scripts.js は共通用として残す。
   * - /fudousan/ の Isotope は fudousan-page.js に一本化する。
   * =========================================================
   */
  if (document.querySelector('.fudousan-page')) return;
...
(function ($) {
  ...
  function initHomeIsotope() {
    /*
   * FUDOUSAN_ISOTOPE_EXCLUDE
   *
   * /fudousan/ のカテゴリータブ・Isotope は、
   * hub/pages/fudousan/js/fudousan-page.js が専用で管理する。
   *
   * 理由:
   * - scripts.js はサイト共通JSで、他ページの .home-posts / .home-cats-selection も扱う。
   * - /fudousan/ でもこの共通 Isotope を動かすと、
   *   fudousan-page.js 側の Isotope と二重初期化になる。
   * - 二重になると、タブクリック時にカードが急に詰まる、
   *   上へ跳ねる、active状態がずれる原因になる。
   *
   * 方針:
   * - scripts.js の Isotope は他ページ用として残す。
   * - /fudousan/ だけはここで return して止める。
   */
  if (document.querySelector('.fudousan-page')) return;
    var $grid = $('.home-posts');
