...
$args = array(
    ...
    'category_name' => 'naigai-tochi', // カテゴリー名をカンマ区切りで指定
);
    ...
    <ul id="sidebar-recent-posts" class="sidebar-posts">
        <?php while ($my_query->have_posts()) : $my_query->the_post();
            ...
            if (has_post_thumbnail()) {
                ...
                $thumbnail_url = get_template_directory_uri() . '/images/noimage.gif'; // サムネイルがない場合のデフォルト画像
            }
            ...
            ?>
            <li id="post-<?php the_ID(); ?>" class="custom-post <?php echo $counter === 1 ? 'first-post' : ''; ?>">
                <?php if ($counter === 1) : ?>
                    <!-- 1番目の投稿：大きなサムネイル -->
                    <div class="recent-thumbnail-box-big" style="position: relative; width: 100%; overflow: hidden;">
                        <a href="<?php the_permalink(); ?>"> <!-- サムネイル画像にリンクを追加 -->
                            <img src="<?php echo esc_url($thumbnail_url); ?>" alt="サムネイル画像" style="width: 100%; height: 100%; object-fit: cover;">
                        </a>
                        <?php if ($is_new) : ?>
                            <div class="ribbon ribbon-top-left"><span>New</span></div>
                        <?php endif; ?>
                        <div class="heart-icon" data-post-id="<?php echo get_the_ID(); ?>" style="position: absolute; top: 10px; right: 10px;">
                ...
                <?php else : ?>
                    <!-- 2番目以降の投稿：小さなサムネイル -->
                    <div class="recent-thumbnail-and-content" style="display: flex; width: 100%; margin-bottom: 0px;">
                        <div class="recent-thumbnail-box-small" style="position: relative; height: 157px; width: 50%; overflow: hidden;">
                            <a href="<?php the_permalink(); ?>"> <!-- サムネイル画像にリンクを追加 -->
                                <img src="<?php echo esc_url($thumbnail_url); ?>" alt="サムネイル画像" style="width: 100%; height: 100%; object-fit: cover;">
                            </a>
                            <?php if ($is_new) : ?>
                        ...
                        <div class="recent-small-content-box" style="width: 50%; padding: 10px ">
                            <h3>
                                <a href="<?php the_permalink(); ?>"><?php echo esc_html($title_excerpt); ?></a> <!-- タイトルにリンクを追加 -->
                            </h3>
                            <div class="more-link-wrapper">
