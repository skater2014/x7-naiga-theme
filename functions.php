 ...
 * 役割:
 * - サイト全体で使う共通CSSだけを読む。
 * - slick / swiper / lite-youtube / lightbox / pannellum / style.css など。
 *
 * 注意:
 * - 民泊CSSはここでは読まない。
 * - 民泊CSS/JSは minpaku/inc/enqueue.php に集約済み。
 * - 不動産CSS/JSは hub/pages/fudousan/inc/assets.php に集約済み。
 * - 家づくりCSS/JSは hub/pages/iezukuri/inc/assets.php 側に集約する。
 *
 * この関数に戻さないもの:
 * - minpaku/common/css/minpaku-base.css
 * - minpaku/common/css/minpaku-single.css
 * - minpaku/common/css/minpaku-booking.css
 * - minpaku/common/css/minpaku-archive.css
 */
...
function themebs_enqueue_scripts()
{
    ...
    /* Moved: /fudousan map modal JS is loaded via hub/pages/fudousan/inc/assets.php. */

    /*
     * Moved:
     * 民泊詳細 / checkout 用 JS は minpaku/inc/enqueue.php へ移動済み。
     * mnpkBooking は minpaku/inc/common-core.php が担当する。
     */
 ...
 * Hub機能の読み込み入口。
 *
 * functions.php を肥大化させないため、Hub専用の読み込みは
 * inc/loaders/hub-loader.php に分離する。
 *
 * 読み込みの流れ:
 * functions.php
 *   ↓
 ...
 *
 * 注意:
 * - functions.php に Hub本体の処理を直接増やさない。
 * - Hub本体は hub/init.php 側で管理する。
 * - Hub用CSS/JSは hub/inc/assets.php 側に集約する。
 * =========================================================
 */
$naigai_hub_loader_bridge = get_template_directory() . '/inc/loaders/hub-loader.php';

if (file_exists($naigai_hub_loader_bridge)) {
 ...
 * =========================================================
 */
// Moved: /fudousan assets are loaded via hub/pages/fudousan/inc/assets.php.
/**
 ...
 * 家づくり機能の読み込み入口。
 *
 * functions.php を肥大化させないため、家づくり専用の読み込みは
 * inc/loaders/iezukuri-loader.php に分離する。
 *
 * 読み込みの流れ:
 * functions.php
 *   ↓
 ...
 *
 * 注意:
 * - functions.php に /iezukuri/ 用CSS/JSの enqueue を直接書かない。
 * - フロントCSS/JSは hub/pages/iezukuri/inc/assets.php 側に集約する。
 * - 管理画面CSS/JSは hub/pages/iezukuri/admin/enqueue.php 側に分ける。
 * =========================================================
 */
$naigai_iezukuri_loader_bridge = get_template_directory() . '/inc/loaders/iezukuri-loader.php';

if (file_exists($naigai_iezukuri_loader_bridge)) {
    require_once $naigai_iezukuri_loader_bridge;
}
 ...
 * 不動産機能の読み込み入口。
 *
 * functions.php を肥大化させないため、不動産専用の読み込みは
 * inc/loaders/fudousan-loader.php に分離する。
 *
 * 読み込みの流れ:
 * functions.php
 *   ↓
 ...
 *
 * 注意:
 * - functions.php に不動産用CSS/JSの enqueue を直接増やさない。
 * - 不動産関連は hub/pages/fudousan/inc/loader.php 側に集約していく。
 * =========================================================
 */
$naigai_fudousan_loader_bridge = get_template_directory() . '/inc/loaders/fudousan-loader.php';
