 ...
 */
$naigai_chatgpt_modal_display_file = get_template_directory() . '/inc/functions/chatgpt-modal-display.php';
if (file_exists($naigai_chatgpt_modal_display_file)) {
    require_once $naigai_chatgpt_modal_display_file;
}
 ...
 * =========================================================
 */
function x7_naiga_theme_setup()
{
    /**
     * Gutenberg / ブロックエディター関連
     */
    ...
    /**
     * 必要なら post-formats を有効化
     */
    // add_theme_support('post-formats', array(
    //     'aside', 'gallery', 'image', 'chat', 'link',
    //     'quote', 'status', 'audio', 'video'
    // ));
}
add_action('after_setup_theme', 'x7_naiga_theme_setup');
 ...
 * =========================================================
 */
function x7_naiga_is_first_post()
{
    global $wp_query;

    return isset($wp_query->current_post) && $wp_query->current_post === 0;
}
 ...
 * =========================================================
 */
function x7_naiga_excerpt_more($more)
{
    return ' ... ';
}
add_filter('excerpt_more', 'x7_naiga_excerpt_more');
...
/**
 * =========================================================
 * ローカル開発用: 画像URLの安全なフォールバック
 * =========================================================
 *
 * 目的:
 * - ローカルに uploads の実ファイルが無い画像は本番URLへ逃がす
 * - ただしローカルで新しく追加した画像は壊さない
 * - upload_dir は触らない
 * - 元画像 / 中間サイズ / srcset を補正する
 */

if (!function_exists('x7_naiga_get_production_hosts')) {
    function x7_naiga_get_production_hosts()
    {
        return array(
            'naigaicorp.net',
            'www.naigaicorp.net',
...
if (!function_exists('x7_naiga_is_local_env')) {
    function x7_naiga_is_local_env()
    {
        ...
        return !in_array($home_host, x7_naiga_get_production_hosts(), true);
    }
}

if (!function_exists('x7_naiga_prod_uploads_baseurl')) {
    function x7_naiga_prod_uploads_baseurl()
    {
        return 'https://naigaicorp.net/wp-content/uploads';
    }
}

if (!function_exists('x7_naiga_local_uploads_baseurl')) {
    function x7_naiga_local_uploads_baseurl()
    {
        return home_url('/wp-content/uploads');
    }
}

if (!function_exists('x7_naiga_get_uploads_relative_path_from_url')) {
    function x7_naiga_get_uploads_relative_path_from_url($url)
    {
        $url = (string) $url;
...
if (!function_exists('x7_naiga_get_local_upload_file_from_relative_path')) {
    function x7_naiga_get_local_upload_file_from_relative_path($relative_path)
    {
        ...
        return WP_CONTENT_DIR . '/uploads/' . $relative_path;
    }
}

if (!function_exists('x7_naiga_get_local_upload_url_from_relative_path')) {
    function x7_naiga_get_local_upload_url_from_relative_path($relative_path)
    {
        $relative_path = ltrim((string) $relative_path, '/');
        ...
        return untrailingslashit(x7_naiga_local_uploads_baseurl()) . '/' . $relative_path;
    }
}

if (!function_exists('x7_naiga_get_prod_upload_url_from_relative_path')) {
    function x7_naiga_get_prod_upload_url_from_relative_path($relative_path)
    {
        $relative_path = ltrim((string) $relative_path, '/');
        ...
        return untrailingslashit(x7_naiga_prod_uploads_baseurl()) . '/' . $relative_path;
    }
}

if (!function_exists('x7_naiga_get_attached_relative_path')) {
    function x7_naiga_get_attached_relative_path($attachment_id)
    {
...
if (!function_exists('x7_naiga_convert_upload_url_to_safe_url')) {
    function x7_naiga_convert_upload_url_to_safe_url($url, $attachment_id = 0)
    {
        if (!x7_naiga_is_local_env()) {
            return $url;
        }

        $relative_path = x7_naiga_get_uploads_relative_path_from_url($url);

        if ($relative_path === '') {
            return $url;
        }

        $requested_local_file = x7_naiga_get_local_upload_file_from_relative_path($relative_path);
        if ($requested_local_file && file_exists($requested_local_file)) {
            return x7_naiga_get_local_upload_url_from_relative_path($relative_path);
        }

        if ($attachment_id) {
            $attached_relative_path = x7_naiga_get_attached_relative_path($attachment_id);

            if ($attached_relative_path !== '') {
                $attached_local_file = x7_naiga_get_local_upload_file_from_relative_path($attached_relative_path);

                if ($attached_local_file && file_exists($attached_local_file)) {
                    return x7_naiga_get_local_upload_url_from_relative_path($attached_relative_path);
                }
            }
        }

        return x7_naiga_get_prod_upload_url_from_relative_path($relative_path);
    }
}

if (!function_exists('x7_naiga_filter_wp_get_attachment_url_safe')) {
    function x7_naiga_filter_wp_get_attachment_url_safe($url, $post_id)
    {
        return x7_naiga_convert_upload_url_to_safe_url($url, $post_id);
    }
}

if (!has_filter('wp_get_attachment_url', 'x7_naiga_filter_wp_get_attachment_url_safe')) {
    add_filter('wp_get_attachment_url', 'x7_naiga_filter_wp_get_attachment_url_safe', 20, 2);
}

if (!function_exists('x7_naiga_filter_wp_get_attachment_image_src_safe')) {
    function x7_naiga_filter_wp_get_attachment_image_src_safe($image, $attachment_id, $size, $icon)
    {
        if (!is_array($image) || empty($image[0])) {
            return $image;
        }

        $image[0] = x7_naiga_convert_upload_url_to_safe_url($image[0], $attachment_id);

        return $image;
    }
}

if (!has_filter('wp_get_attachment_image_src', 'x7_naiga_filter_wp_get_attachment_image_src_safe')) {
    add_filter('wp_get_attachment_image_src', 'x7_naiga_filter_wp_get_attachment_image_src_safe', 20, 4);
}

if (!function_exists('x7_naiga_filter_wp_calculate_image_srcset_safe')) {
    function x7_naiga_filter_wp_calculate_image_srcset_safe($sources, $size_array, $image_src, $image_meta, $attachment_id)
    {
        if (!is_array($sources)) {
            return $sources;
        }
        ...
        foreach ($sources as $width => $source) {
            if (!empty($source['url'])) {
                $sources[$width]['url'] = x7_naiga_convert_upload_url_to_safe_url($source['url'], $attachment_id);
            }
        }
...
if (!has_filter('wp_calculate_image_srcset', 'x7_naiga_filter_wp_calculate_image_srcset_safe')) {
    add_filter('wp_calculate_image_srcset', 'x7_naiga_filter_wp_calculate_image_srcset_safe', 20, 5);
}
 ...
 * =========================================================
 */
function x7_naiga_child_category_link_custom($query)
{
    if (
        isset($query['category_name'], $query['name']) &&
        $query['category_name'] === 'category-parent' &&
        $query['name'] === 'category-child'
    ) {
        unset($query['name']);
...
add_filter('request', 'x7_naiga_child_category_link_custom');
...
/**
 * =========================================================
 * editor / admin / front のCSS読込を整理
 * =========================================================
 *
 * ルール:
 * - editor-style.css → ブロックエディター用
 * - admin-style.css  → 管理画面全体用
 * - editor-style.css をフロントでも読む
 *   ただし front で効かせたいセレクタは CSS 側で合わせる
 */

/**
 * ファイル更新時刻をバージョンに使う
 */
if (!function_exists('naigai_asset_ver')) {
    function naigai_asset_ver($relative_path)
    {
...
/**
 * =========================================================
 * assets / editor / admin / frontend 読み込み整理版
 * =========================================================
 *
 * 置換ルール:
 * - 既存の重複した themebs_enqueue_styles()
 * - 既存の重複した themebs_enqueue_scripts()
 * - 途中で切れた壊れた asset ブロック
 * は削除して、このブロックだけを残す
 */
 ...
 * =========================================================
 */
function x7_naiga_setup_editor_styles()
{
    add_theme_support('editor-styles');
    add_editor_style('css/editor-style.css');
}
add_action('after_setup_theme', 'x7_naiga_setup_editor_styles');
 ...
 * =========================================================
 */
function naigai_enqueue_post_editor_assets($hook_suffix)
{
    if ($hook_suffix !== 'post.php' && $hook_suffix !== 'post-new.php') {
        return;
    }

    $editor_js_rel = '/js/editor-style.js';

    if (file_exists(get_template_directory() . $editor_js_rel)) {
        wp_enqueue_script(
            'naigai-editor-script',
            get_template_directory_uri() . $editor_js_rel,
            array(),
            naigai_asset_ver($editor_js_rel),
            true
        );
    }
}
add_action('admin_enqueue_scripts', 'naigai_enqueue_post_editor_assets');
 ...
 * =========================================================
 */
function naigai_enqueue_admin_assets()
{
    $admin_css_rel = '/css/admin-style.css';

    if (file_exists(get_template_directory() . $admin_css_rel)) {
        wp_enqueue_style(
            'naigai-admin-style',
            get_template_directory_uri() . $admin_css_rel,
            array(),
            naigai_asset_ver($admin_css_rel)
        );
    }
}
add_action('admin_enqueue_scripts', 'naigai_enqueue_admin_assets');

/**
 * =========================================================
 * フロント側でも editor-style.css を読む
 * =========================================================
 */
function naigai_enqueue_front_editor_style()
{
    $editor_css_rel = '/css/editor-style.css';

    if (file_exists(get_template_directory() . $editor_css_rel)) {
        wp_enqueue_style(
            'naigai-front-editor-style',
            get_template_directory_uri() . $editor_css_rel,
            array(),
            naigai_asset_ver($editor_css_rel)
        );
    }
}
add_action('wp_enqueue_scripts', 'naigai_enqueue_front_editor_style');
 ...
 * =========================================================
 */
function naigai_enqueue_jquery_ui()
{
...
add_action('wp_enqueue_scripts', 'naigai_enqueue_jquery_ui');
 ...
 * =========================================================
 */
function naigai_enqueue_google_fonts()
{
...
add_action('wp_enqueue_scripts', 'naigai_enqueue_google_fonts');
 ...
 * =========================================================
 */
function naigai_enqueue_font_awesome()
{
    $fontawesome_rel = '/css/all.css';

    if (file_exists(get_template_directory() . $fontawesome_rel)) {
        wp_enqueue_style(
            'naigai-font-awesome',
            get_template_directory_uri() . $fontawesome_rel,
            array(),
            naigai_asset_ver($fontawesome_rel)
        );
    }
}
add_action('wp_enqueue_scripts', 'naigai_enqueue_font_awesome');
 ...
 * - テーマ共通CSSを読む
 * - 民泊共通CSSを読む
 * - 民泊アーカイブ時だけ archive 専用CSSを読む
 * - 民泊詳細時だけ single 専用CSSを読む
 * - 民泊詳細 / checkout の予約UIは booking 専用CSSを読む
 *
 * 今回の方針:
 * - minpaku.css
 *   → ボタン / pills / フォーム / モーダルなどの共通UI
 * - minpaku-single.css
 *   → 詳細ページ本文 / ギャラリー / 2カラム
 * - minpaku-booking.css
 *   → 予約カード / カレンダー / checkout / payment UI
 */
function themebs_enqueue_styles()
{
     ...
     */
    $style_files = array(
        'slick-slider-css'     => '/css/slick.css',
        'swiper-style'         => '/css/swiper-bundle.css',
        'slicknav-custom'      => '/css/slicknav.css',
        'lite-yt-embed-css'    => '/css/lite-yt-embed.css',
        'lite-vimeo-embed-css' => '/css/lite-vimeo-embed.css',
        'lightbox-css'         => '/css/lightbox.css',
        'loan-simulation-css'  => '/css/loan-simulation.css',
        'pannellum-css'        => '/css/pannellum.css',
    ...
    foreach ($style_files as $handle => $relative_path) {
        ...
        if (file_exists($absolute_path)) {
            wp_enqueue_style(
                ...
                array(),
                naigai_asset_ver($relative_path)
            );
        }
    }
     ...
     * - 民泊詳細
     *
     * checkout endpoint も通常は同じ minpaku singular 文脈で読む想定
     */
    $is_minpaku_archive = is_post_type_archive('minpaku');
     ...
     * - モーダル土台
     */
/* old minpaku.css enqueue disabled */
     ...
     *
     * DISABLED:
     * 民泊CSSは minpaku/inc/enqueue.php に一本化。
     * functions.php 側で読むと handle が二重化するため停止。
     */
    /**
     * ---------------------------------------------------------
     * 民泊詳細ページ専用CSS / 予約UI専用CSS
     * ---------------------------------------------------------
     *
     * DISABLED:
     * minpaku-single.css / minpaku-booking.css は
     * minpaku/inc/enqueue.php の naigai_minpaku_enqueue_assets() で読む。
     */
}
...
function themebs_enqueue_scripts()
{
    ...
    $enqueue = function ($handle, $relpath, $deps = array('jquery'), $in_footer = true) use ($theme_uri) {
        if (file_exists(get_template_directory() . $relpath)) {
            wp_enqueue_script(
                $handle,
                $theme_uri . $relpath,
                $deps,
                naigai_asset_ver($relpath),
                $in_footer
            );
        }
    };
     ...
     * 共通JS
     */
    $enqueue('image-handler', '/js/imageHandler.js', array('jquery'), true);
    $enqueue('event-listeners', '/js/eventListeners.js', array('jquery', 'image-handler'), true);
    $enqueue('ajax-handler', '/js/ajaxHandler.js', array('jquery'), true);

    wp_localize_script('ajax-handler', 'customAjax', array(
     ...
     * 共通ライブラリ
     */
    $enqueue('datatables-min-js', '/js/datatables.min.js', array('jquery'), true);
    $enqueue('fullcalendar-min-js', '/js/index.global.min.js', array(), true);

    $enqueue('swiper-slider', '/js/swiper-bundle.min.js', array(), true);
    $enqueue('slicknav', '/js/jquery.slicknav.min.js', array('jquery'), true);
    $enqueue('slick-slider-js', '/js/slick.min.js', array('jquery'), true);

    $enqueue('imagesloaded', '/js/imagesloaded.pkgd.min.js', array(), true);
    $enqueue('isotope', '/js/isotope.pkgd.min.js', array('imagesloaded'), true);
    $enqueue('dotdot', '/js/jquery.dotdotdot.min.js', array('jquery'), true);

    $enqueue('lite-yt-embed-js', '/js/lite-yt-embed.js', array(), true);
    $enqueue('lite-vimeo-embed-js', '/js/lite-vimeo-embed.js', array(), true);
    $enqueue('lightbox', '/js/lightbox.min.js', array(), true);
    $enqueue('clipboard', '/js/clipboard.min.js', array(), true);
    $enqueue('loan-simulation-js', '/js/loan-simulation.js', array('jquery'), true);
    $enqueue('calendar-js', '/js/calendar.js', array('jquery', 'fullcalendar-min-js'), true);

    /**
     * Pannellum
     */
    $enqueue('libpannellum', '/js/libpannellum.js', array(), true);
    $enqueue('pannellum', '/js/pannellum.js', array('libpannellum'), true);
     ...
     * テーマ共通
     */
    $enqueue('theme-scripts', '/js/scripts.js', array('jquery'), true);

    /**
     * トップページ専用
     */
    if (is_front_page()) {
        $enqueue('naigai-ai-chat', '/js/openai-chat-modal.js', array('jquery'), true);
    ...
    /* =========================================================
     * fudousan map modal script target
     * 役割:
     * - Google Map モーダルJSは /fudousan だけで読み込む
     * - トップページでは読み込まない
     * - /fudousan の地図ボタン:
     *   .google-location-trigger / [data-map-open] / data-map-html
     *   を js/front-map-modal.js が拾う
     * ========================================================= */
    if (
        is_page('fudousan')
        || is_page_template('template-realestate-hub.php')
    ) {
        $enqueue('naigai-front-map-modal', '/js/front-map-modal.js', array(), true);
    }
    /* =========================================================
     * fudousan map modal script target end
     * ========================================================= */

    /**
     * 民泊詳細ページ専用
     */
    if (is_singular('minpaku') || (function_exists('mnpk_is_checkout_endpoint') && mnpk_is_checkout_endpoint())) {
        wp_enqueue_script(
            'stripe-js',
        ...
        $enqueue(
            'minpaku-single-js',
            '/minpaku/common/js/minpaku-single.js',
            array(
                'jquery',
                'swiper-slider',
                'pannellum',
...
function pannellum_metabox()
{
    add_meta_box(
        ...
        ['post', 'house', 'recruitment', 'blog', 'minpaku'],                   // メタボックスを表示する投稿タイプ
        'normal',                            // メタボックスの表示位置
        'default'                            // メタボックスの優先度
    );
}
...
function pannellum_metabox_save($post_id)
{
    ...
    if (!in_array($post_type, ['house', 'post', 'blog', 'recruitment', 'minpaku'], true)) {
        return $post_id;
    }
 ...
 * =========================================================
 *
 * ここでは Hub 本体を 1 回だけ読み込みます。
 *
 * 初心者向けの考え方:
 * - WordPress は functions.php を自動で読む
 * - hub/ フォルダの中身は、自動では読まれない
 * - そのため、functions.php から hub/init.php を読み込んで
 *   Hub の機能を起動する必要がある
 *
 * 役割分担:
 * - functions.php        = テーマ全体の入口
 * - hub/init.php         = Hub の起動ファイル
 * - hub/inc/common.php   = 共通関数
 * - hub/inc/context.php  = body_class などの文脈判定
 * - hub/inc/admin.php    = 管理画面のメタキー入力欄
 * - hub/inc/assets.php   = Hub の CSS / JS 読み込み
 *
 * 重要:
 * - Hub の本体処理は functions.php にベタ書きしない
 * - Hub 関連は hub/ 配下で管理する
 * - functions.php には「Hub を起動する入口」だけを残す
 */
require_once get_template_directory() . '/hub/init.php';
...
// サービス一覧 固定ページ メタボックス
require_once get_template_directory() . '/inc/functions/functions-nasu-guide.php';

// 会社概要ページ 固定ページ メタボックス
require_once get_template_directory() . '/inc/functions/functions-page-company.php';


// 固定ページ　6枚
// require_once get_template_directory() . '/inc/functions/functions-minpaku-layout-assets.php';

// checkoutページ呼び出し機能

// リノベ中古住宅　メタボックス
require_once get_template_directory() . '/inc/functions/functions-page-used-renovation.php';

// 在来工法ページ 固定ページ メタボックス
require_once get_template_directory() . '/inc/functions/functions-page-zairai.php';

// 北米工法ページ 固定ページ メタボックス
require_once get_template_directory() . '/inc/functions/functions-page-wakugumi.php';

// 施工事例　固定ページ　メタボックス
require_once get_template_directory() . '/inc/functions/functions-page-sekoujirei.php';
...
// 資料請求　固定ページ　フォーム
require_once get_template_directory() . '/inc/functions/functions-customer-info-form.php';
...
function create_my_custom_post_types()
{
    ...
    register_post_type('house', array(
        ...
        'labels' => array(
            ...
            'singular_name'         => '住宅物件',
            'add_new'               => '新規住宅を追加',
            'add_new_item'          => '新しい住宅物件を追加',
            'edit_item'             => '住宅物件を編集',
            'new_item'              => '新しい住宅物件',
            'view_item'             => '住宅物件を表示',
            'search_items'          => '住宅物件を検索',
            'not_found'             => '住宅物件は見つかりませんでした。',
            'not_found_in_trash'    => 'ゴミ箱に住宅物件はありませんでした。',
            'all_items'             => '住宅情報一覧',
            'archives'              => '住宅情報アーカイブ',
            'menu_name'             => '住宅情報',
            'name_admin_bar'        => '住宅物件',
    ...
    register_post_type('blog', array(
        ...
        'labels' => array(
            ...
            'singular_name'         => 'コラム記事',
            'add_new'               => '新規追加',
            'add_new_item'          => '新しいコラム記事を追加',
            'edit_item'             => 'コラム記事を編集',
            'new_item'              => '新しいコラム記事',
            'view_item'             => 'コラム記事を表示',
            'search_items'          => 'コラム記事を検索',
            'not_found'             => '記事が見つかりませんでした',
            'not_found_in_trash'    => 'ゴミ箱に記事がありません',
            'all_items'             => 'コラム一覧',
            'archives'              => '不動産コラム一覧',
            'menu_name'             => '不動産コラム',
            'name_admin_bar'        => 'コラム記事',
        ),
        'public' => true,
        'has_archive' => 'fudosan-column',
        'hierarchical' => false,
        'menu_position' => 6,
    ...
    register_taxonomy('house-type', array('house', 'post'), array(
        ...
        'labels' => array(
            'name'              => '住宅種別',
            'singular_name'     => '住宅種別',
            'popular_items'     => 'よく使う住宅種別',
            'edit_item'         => '住宅種別を編集',
            'add_new_item'      => '新規住宅種別を追加',
            'search_items'      => '住宅種別を検索',
            'all_items'         => '住宅種別一覧',
            'menu_name'         => '住宅種別',
        ),
        'public' => true,
    ...
    register_taxonomy('parent_taxonomy', array('house', 'post'), array(
        ...
        'labels' => array(
            'name'              => '物件分類',
            'singular_name'     => '物件分類',
            'search_items'      => '物件分類を検索',
            'all_items'         => '物件分類一覧',
            'edit_item'         => '物件分類を編集',
            'add_new_item'      => '新規物件分類を追加',
            'menu_name'         => '物件分類',
        ),
        'public' => true,
        'hierarchical' => true,
...
/**
 * ------------------------------------------------------------
 * house 投稿タイプで通常タグを使えるようにする
 * ------------------------------------------------------------
 */
if (!function_exists('enable_tags_for_house')) {
    function enable_tags_for_house()
    {
        register_taxonomy_for_object_type('post_tag', 'house');
    }
}
add_action('init', 'enable_tags_for_house');
 ...
 * ------------------------------------------------------------
 */
if (!function_exists('my_custom_post_type_archive_title')) {
...
add_filter('get_the_archive_title', 'my_custom_post_type_archive_title');
...
{
    $post_types = array('post', 'genshin', 'house', 'artifacts', 'minpaku'); // 使用するすべての投稿タイプ　別にこのように必ず記載
        ...
        foreach ($custom_fields as $field_name => $label) {
            ...
            if (strpos($field_name, 'Image') !== false) {
                add_meta_box(
                    ...
                    array('post', 'house', 'minpaku'),  // 投稿タイプ
                    'normal',  // 表示位置
                    'high',  // 優先度
...
function save_custom_fields_data($post_id)
{
    ...
    if (!in_array($post_type, ['post', 'house', 'minpaku'])) return;
...
function add_custom_fields_meta_boxes()
{
    add_meta_box(
        ...
        array('post', 'house',),  // 'post' と 'house' と 'minpaku' のカスタム投稿タイプに追加
        'normal', // メタボックスの位置
        'high' // メタボックスの優先度
    );
}
...
function add_media_selection_meta_box()
{
    ...
    add_meta_box(
        ...
        array('post', 'page', 'house', 'minpaku'), // メタボックスを表示する投稿タイプ
        'normal', // メタボックスの位置
        'default' // メタボックスの優先度
    );
}
 ...
 *
 * 役割:
 * - 管理画面「外観 > メニュー」に割り当て先を表示する
 * - 共通 / 不動産 / 民泊 で分けて管理する
 */
function naigai_register_theme_menus()
{
    register_nav_menus(array(
        // =====================================================
        // 共通
        // =====================================================
        'header-menu-pc'             => 'PC Header Menu',
        'header-menu-mobile'         => 'Mobile Header Menu',
        'side-menu'                  => 'Common Side Menu',

        // =====================================================
        // 不動産用
        // =====================================================
        'fudosan-side-menu'          => 'Fudosan Side Menu',

        // =====================================================
        // 民泊用
        // =====================================================
        'minpaku-header-menu-pc'     => 'Minpaku Header Menu PC',
        'minpaku-header-menu-mobile' => 'Minpaku Header Menu Mobile',
        'minpaku-side-menu'          => 'Minpaku Stay Side Menu',
    ));
}
 ...
 * - /naigai-construction
 * はここに入れない
 * これらは my_custom_menu() 側で別分岐する
 */
function naigai_is_fudosan_context()
{
    return (
        is_post_type_archive('house') ||
        is_singular('house') ||
        is_page('nasu-jutaku')
    );
}
 ...
 *
 * 役割:
 * - まず primary の location を表示
 * - 未割当なら fallback の location を表示
 * - menu 名の直指定はしない
 */
function naigai_render_sidebar_menu($primary_location, $fallback_location = '')
{
    $args = array(
    ...
    if (!empty($primary_location) && has_nav_menu($primary_location)) {
        $args['theme_location'] = $primary_location;
        wp_nav_menu($args);
        return;
    }

    if (!empty($fallback_location) && has_nav_menu($fallback_location)) {
 ...
 * 仕様:
 * - 民泊                              → minpaku 5件
 * - naigai-tochi ページ / カテゴリー  → 土地カテゴリ投稿 5件
 * - naigai-construction ページ / カテゴリー → 建設カテゴリ投稿 5件
 * - nasu-jutaku / house 系            → house 5件
 * - その他                            → 通常投稿 5件
 *
 * 重要:
 * - 土地 / 建設は house 分岐より前に判定する
 * - そうしないと house 5件に吸われる
 * - 保存値は使わない。件数は PHP 側で固定
 */
function my_custom_menu()
{
    ...
    if (naigai_is_minpaku_context()) {
        naigai_render_sidebar_menu('minpaku-side-menu', 'side-menu');
     ...
     * - カテゴリー naigai-tochi
     */
    if (is_page('naigai-tochi') || is_category('naigai-tochi')) {
        naigai_render_sidebar_menu('fudosan-side-menu', 'side-menu');
     ...
     *
     * 対象:
     * - 固定ページ naigai-construction
     * - カテゴリー naigai-construction
     */
    if (is_page('naigai-construction') || is_category('naigai-construction')) {
        naigai_render_sidebar_menu('fudosan-side-menu', 'side-menu');
     ...
     * =====================================================
     */
    naigai_render_sidebar_menu('side-menu', 'fudosan-side-menu');
 ...
 *
 * 重要:
 * - public 側で the_widget('Latest_Posts_Widget') を使うため残す
 * - 管理画面の保存値を使わなくても登録自体は残してよい
 */
 ...
 *
 * 役割:
 * - Latest widget の画像を返す
 *
 * 優先順:
 * 1. アイキャッチ
 * 2. page_featured_type / page_video_id の動画画像
 * 3. noimage.gif
 */
function naigai_get_sidebar_thumbnail_url($post_id)
{
    $image = get_the_post_thumbnail_url($post_id, 'full');

    if (!$image) {
        $video_type = trim((string) get_post_meta($post_id, 'page_featured_type', true));
        $video_id   = trim((string) get_post_meta($post_id, 'page_video_id', true));

        if ($video_type === 'youtube' && $video_id !== '') {
            if (preg_match('~(?:youtube\.com/(?:watch\?v=|embed/|shorts/)|youtu\.be/)([A-Za-z0-9_-]{11})~', $video_id, $matches)) {
                $video_id = $matches[1];
            }

            $image = 'https://img.youtube.com/vi/' . rawurlencode($video_id) . '/hqdefault.jpg';
 ...
 *
 * 役割:
 * - 渡された WP_Query を使って最新一覧を描画する
 * - 通常投稿 / 民泊 / 住宅 すべてに対応する
 * - アイキャッチが無くても動画サムネイルを補完する
 *
 * 重要:
 * - 保存値は使わない
 * - public 側表示用として class は残す
 */
class Latest_Posts_Widget extends WP_Widget
{
    public function __construct()
    {
        parent::__construct(
            ...
            'Latest Posts Widget',
            array(
                'description' => 'Display latest posts with thumbnails',
            )
        );
    }
    ...
    public function widget($args, $instance)
    {
        ...
        $posts = isset($instance['posts']) ? $instance['posts'] : null;

        if ($posts instanceof WP_Query && $posts->have_posts()) {
            ...
            while ($posts->have_posts()) {
                ...
                $post_id   = get_the_ID();
                $thumb_url = naigai_get_sidebar_thumbnail_url($post_id);

                echo '<div class="custom-widget-item">';

                echo '<a href="' . esc_url(get_permalink()) . '" title="' . esc_attr(get_the_title()) . '">';
                echo '<img width="150" height="100" src="' . esc_url($thumb_url) . '" class="attachment-home-thumb size-home-thumb wp-post-image" alt="' . esc_attr(get_the_title()) . '" decoding="async" loading="lazy">';
                echo '</a>';
    ...
    {
        echo '<p>このウィジェットは PHP 側で固定表示しています。管理画面からの保存は使用しません。</p>';
    }
...
function add_google_map_metabox()
{
    ...
    $post_types = array('post', 'house', 'minpaku'); // カスタム投稿タイプ名を 'house' に変更
...
function save_google_map_metabox_data($post_id)
{
    ...
    $post_type = get_post_type($post_id);
    $post_types = array('post', 'house', 'minpaku'); // 対応する投稿タイプをリスト（'house' に変更）
 ...
 * 物件の現況ラベル変換
 */
function format_satei_property_status_label($value)
{
    $map = array(
 ...
 * 売却希望時期ラベル変換
 */
function format_satei_sale_period_label($value)
{
    $map = array(
        '1_month'   => '1ヶ月以内',
        '3_months'  => '3ヶ月以内',
 ...
 * 査定方法ラベル変換
 */
function format_satei_valuation_method_label($value)
{
    $map = array(
        'desk'  => '机上査定（簡易）',
        'visit' => '訪問査定（詳細）',
 ...
 * 査定理由ラベル変換
 */
function format_satei_sale_reason_label($value)
{
    $map = array(
        'inheritance' => '相続',
        'relocation'  => '住み替え',
...
function handle_store_reservation_submit()
{
     ...
     * ----------------------------------------------------- */
    if (
        !isset($_POST['store_reservation_nonce']) ||
        !wp_verify_nonce($_POST['store_reservation_nonce'], 'store_reservation_action')
    ) {
     ...
     * フォーム種別判定
     * -----------------------------------------------------
     * page_url / form_type を優先しつつ、
     * URL 文字列でも判定する
     * ----------------------------------------------------- */
    $page_url  = !empty($form_data['page_url']) ? esc_url_raw($form_data['page_url']) : ($_SERVER['HTTP_REFERER'] ?? '');
    $form_type = !empty($form_data['form_type']) ? sanitize_text_field($form_data['form_type']) : '';

    $is_recruitment = ($form_type === 'recruitment') || (strpos($page_url, '/recruitment') !== false);
    $is_satei       = ($form_type === 'satei') || (strpos($page_url, '/satei') !== false);
    ...
    $katakana   = trim($last_kana . ' ' . $first_kana);
    ...
    $phone    = !empty($form_data['phone']) ? sanitize_text_field($form_data['phone']) : '';
    $postcode = !empty($form_data['postcode']) ? sanitize_text_field($form_data['postcode']) : '';
    $address  = !empty($form_data['address']) ? sanitize_textarea_field($form_data['address']) : '';
     ...
     * 売却査定用の各項目
     * ----------------------------------------------------- */
    $raw_sale_price = isset($form_data['sale_price']) ? sanitize_text_field($form_data['sale_price']) : '';
    $sale_price = ($raw_sale_price !== '' && $raw_sale_price !== '未設定') ? $raw_sale_price . '万円' : '';

    $property_status_raw  = !empty($form_data['property_status']) ? sanitize_text_field($form_data['property_status']) : '';
    $sale_reason_raw      = !empty($form_data['sale_reason']) ? sanitize_text_field($form_data['sale_reason']) : '';
    $sale_reason_text     = !empty($form_data['sale_reason_text']) ? sanitize_textarea_field($form_data['sale_reason_text']) : '';
    $property_postcode    = !empty($form_data['property_postcode']) ? sanitize_text_field($form_data['property_postcode']) : '';
    $property_address     = !empty($form_data['property_address']) ? sanitize_textarea_field($form_data['property_address']) : '';
    $sale_period_raw      = !empty($form_data['sale_period']) ? sanitize_text_field($form_data['sale_period']) : '';
    $valuation_method_raw = !empty($form_data['valuation_method']) ? sanitize_text_field($form_data['valuation_method']) : '';

    /* 表示用ラベルへ変換 */
    $property_status  = $property_status_raw ? format_satei_property_status_label($property_status_raw) : '';
    $sale_reason      = $sale_reason_raw ? format_satei_sale_reason_label($sale_reason_raw) : '';
    $sale_period      = $sale_period_raw ? format_satei_sale_period_label($sale_period_raw) : '';
    $valuation_method = $valuation_method_raw ? format_satei_valuation_method_label($valuation_method_raw) : '';
     ...
     * 売却査定では本文に使わない
     * ----------------------------------------------------- */
    $property_id    = !empty($form_data['property_id']) ? absint($form_data['property_id']) : 0;
    $property_title = !empty($form_data['property_title']) ? sanitize_text_field($form_data['property_title']) : '';
    $property_price = !empty($form_data['property_price']) ? sanitize_text_field($form_data['property_price']) : '';
    $property_label = !empty($form_data['property_label'])
        ? sanitize_text_field($form_data['property_label'])
        : ($is_recruitment ? '採用情報' : '物件情報');
    ...
    /* お客様情報 */
    $message .= "<h3>お客様情報</h3>";
    $message .= "<p><strong>お名前:</strong> " . esc_html($name ?: '未入力') . "</p>";
    $message .= "<p><strong>カタカナ:</strong> " . esc_html($katakana ?: '未入力') . "</p>";
    $message .= "<p><strong>メールアドレス:</strong> " . esc_html($email ?: '未入力') . "</p>";
    $message .= "<p><strong>電話番号:</strong> " . esc_html($phone ?: '未入力') . "</p>";
    $message .= "<p><strong>郵便番号:</strong> " . esc_html($postcode ?: '未入力') . "</p>";
    $message .= "<p><strong>住所:</strong> " . nl2br(esc_html($address ?: '未入力')) . "</p>";

    /* 売却査定 */
    if ($is_satei) {
        $message .= "<h3>お客様のご希望の売却査定の情報</h3>";

        if ($property_postcode) {
            $message .= "<p><strong>査定物件の郵便番号:</strong> " . esc_html($property_postcode) . "</p>";
        }

        if ($property_address) {
            $message .= "<p><strong>査定物件の住所:</strong><br>" . nl2br(esc_html($property_address)) . "</p>";
        }

        if ($property_status) {
            $message .= "<p><strong>物件の現況:</strong> " . esc_html($property_status) . "</p>";
        }

        /* セレクト理由と自由記述を整形 */
        $sale_reason_display = '';
        if ($sale_reason && $sale_reason_text) {
            $sale_reason_display = $sale_reason . ' / ' . $sale_reason_text;
        } elseif ($sale_reason) {
            $sale_reason_display = $sale_reason;
        } elseif ($sale_reason_text) {
            $sale_reason_display = $sale_reason_text;
        }

        if ($sale_reason_display) {
            $message .= "<p><strong>査定を希望する理由:</strong><br>" . nl2br(esc_html($sale_reason_display)) . "</p>";
        }

        if ($sale_price) {
            $message .= "<p><strong>売却希望価格:</strong> " . esc_html($sale_price) . "</p>";
        }

        if ($sale_period) {
            $message .= "<p><strong>売却希望時期:</strong> " . esc_html($sale_period) . "</p>";
        }

        if ($valuation_method) {
            $message .= "<p><strong>査定方法:</strong> " . esc_html($valuation_method) . "</p>";
        }

        if ($building_area) {
            $message .= "<p><strong>建物面積:</strong> " . esc_html($building_area) . "</p>";
        }

        if ($land_area) {
            $message .= "<p><strong>土地面積:</strong> " . esc_html($land_area) . "</p>";
        }

        if ($year_built) {
            $message .= "<p><strong>築年数:</strong> " . esc_html($year_built) . "</p>";
        }

        if ($floor_plan) {
            $message .= "<p><strong>間取り:</strong> " . esc_html($floor_plan) . "</p>";
        }

        if ($rental_income) {
            $message .= "<p><strong>賃料収入:</strong> " . esc_html($rental_income) . "</p>";
        }
        ...
        /* 来店予約 / 面接予約 */
        $message .= "<h3>" . esc_html($property_label) . "</h3>";

        if ($property_id) {
            $message .= "<p><strong>ID:</strong> " . esc_html($property_id) . "</p>";
        }

        if ($property_title) {
            $message .= "<p><strong>タイトル:</strong> " . esc_html($property_title) . "</p>";
        }

        if ($property_price !== '') {
            $message .= "<p><strong>価格:</strong> " . esc_html($property_price) . "</p>";
        }

        $message .= "<h3>" . ($is_recruitment ? '面接予約情報' : '来店予約情報') . "</h3>";
        $message .= "<p><strong>来店日:</strong> " . esc_html($visit_date ?: '未入力') . "</p>";
        $message .= "<p><strong>時間帯:</strong> " . esc_html($time_slot ?: '未入力') . "</p>";
    }

    /* 送信元情報 */
    $message .= "<hr>";
    $message .= "<p><strong>送信元URL:</strong> " . esc_html($page_url) . "</p>";
    $message .= "<p><strong>送信元種別:</strong> " . esc_html(!empty($form_data['from_modal']) ? $form_data['from_modal'] : '不明') . "</p>";
    $message .= "</body></html>";
    ...
    error_log('【DB登録テスト】データ挿入準備: ' . print_r(array(
        'type'    => $type,
        'name'    => $name,
        'email'   => $email,
        'phone'   => $phone,
     ...
     * ----------------------------------------------------- */
    if ($mail_ok) {
        wp_send_json_success(array('message' => '送信が完了しました。'));
    } else {
...
// 管理画面ページ　来店予約フォームからの顧客CSV ダウンロード　予約　売却査定　データ申し込み
 ...
 * 役割:
 * - 総合窓口 / 不動産とは別に、注文住宅だけのナビを割り当てる
 * ========================================================= */
if (!function_exists('naigai_register_customhome_menu_location')) {
    function naigai_register_customhome_menu_location() {
        register_nav_menu('customhome-page-menu', 'Custom Home Page Menu');
    }
    add_action('after_setup_theme', 'naigai_register_customhome_menu_location');
}
 ...
 */
if (is_admin()) {
    $naigai_frontpage_admin = get_template_directory() . '/hub/pages/frontpage/admin/frontpage-admin.php';
    if (file_exists($naigai_frontpage_admin)) {
        require_once $naigai_frontpage_admin;
    }
}

/* =========================================================
 * /fudousan admin settings
 * ========================================================= */
$fudousan_admin_file = get_template_directory() . '/hub/pages/fudousan/admin/fudousan-admin.php';
if (file_exists($fudousan_admin_file)) {
    require_once $fudousan_admin_file;
}

/**
 * ChatGPTモーダルをフロントページの footer に出力する
 * naigai_render_chatgpt_modal() 側で is_front_page() 判定しているので、
 * ここでは hook するだけでよい。
 */
...
/* === /fudousan reservation modal ajax override START === */
add_action('init', function () {
    if (!function_exists('wp_doing_ajax') || !wp_doing_ajax()) {
        return;
    }

    remove_all_actions('wp_ajax_naigai_get_reservation_modal_data');
    remove_all_actions('wp_ajax_nopriv_naigai_get_reservation_modal_data');

    add_action('wp_ajax_naigai_get_reservation_modal_data', 'naigai_fudo_ajax_reservation_modal_data', 1);
    add_action('wp_ajax_nopriv_naigai_get_reservation_modal_data', 'naigai_fudo_ajax_reservation_modal_data', 1);
}, 999);

if (!function_exists('naigai_fudo_ajax_meta_first')) {
    function naigai_fudo_ajax_meta_first($post_id, $keys) {
...
if (!function_exists('naigai_fudo_ajax_numeric')) {
    function naigai_fudo_ajax_numeric($value) {
        if (is_array($value)) {
            return 0;
        }
...
if (!function_exists('naigai_fudo_ajax_has_sold_term')) {
    function naigai_fudo_ajax_has_sold_term($post_id) {
        ...
        foreach ($taxes as $tax) {
            ...
            foreach ($terms as $term) {
                $text = $term->slug . ' ' . $term->name;
                if (mb_stripos($text, '売却済') !== false || mb_stripos($text, '成約済') !== false || mb_stripos($text, 'sold') !== false) {
                    return true;
                }
            }
        }
...
if (!function_exists('naigai_fudo_ajax_price_label')) {
    function naigai_fudo_ajax_price_label($post_id) {
        $raw = naigai_fudo_ajax_meta_first($post_id, array(
            'NewPrice',
            'Price',
            '価格',
        ...
        $sold = naigai_fudo_ajax_meta_first($post_id, array(
            'sold-out',
            'sold_out',
            'SoldOut',
            '_sold_out',
        ...
        if (
            naigai_fudo_ajax_has_sold_term($post_id)
            || ($sold !== '' && !in_array(strtolower((string) $sold), array('0', 'no', 'false', 'off', '販売中'), true))
            || (trim((string) $raw) !== '' && naigai_fudo_ajax_numeric($raw) <= 0)
            || preg_match('/売却済|成約済/u', (string) $raw)
        ) {
            return '売却済み';
        }
        ...
        if (preg_match('/万円|円/u', $raw)) {
            return $raw;
        }

        $num = naigai_fudo_ajax_numeric($raw);
        return $num > 0 ? number_format($num) . '万円' : '売却済み';
    }
}

if (!function_exists('naigai_fudo_ajax_reservation_modal_data')) {
    function naigai_fudo_ajax_reservation_modal_data() {
        ...
        wp_send_json_success(array(
            ...
            'title'        => get_the_title($post_id),
            'thumbnail'    => $thumbnail,
            'price'        => naigai_fudo_ajax_price_label($post_id),
...
/* === /fudousan reservation modal ajax override END === */


/* 固定ページ専用CSS loader */
require_once get_template_directory() . '/inc/functions/page-assets-loader.php';

/**
 * =========================================================
 * Frontpage Direct V2 assets
 *
 * front-page.php が template-parts/hub/front-page-v2.php を読むため、
 * fp-* 系CSS/JSをフロントページだけで読み込む。
 * =========================================================
 */
add_action('wp_enqueue_scripts', function () {
    ...
    if (file_exists($theme_dir . $css)) {
        wp_enqueue_style(
            'naigai-frontpage-v2',
            $theme_uri . $css,
            array('theme-style', 'swiper-style'),
...
/**
 * =========================================================
 * FUDOUSAN PAGE ASSETS FIX START
 *
 * /fudousan 専用CSS/JSを固定読み込み。
 * フロントページには影響させない。
 * =========================================================
 */
add_action('wp_enqueue_scripts', function () {
...
/**
 * FUDOUSAN PAGE ASSETS FIX END
 */

/**
 * =========================================================
 * CONSTRUCTION HUB TO IEZUKURI REDIRECT START
 *
 * 旧URL /construction-hub/... を新URL /iezukuri/... へ転送。
 * 既存リンク・検索エンジン・ブックマーク対策。
 * =========================================================
 */
add_action('template_redirect', function () {
    ...
    $map = array(
        '/construction-hub'                => '/iezukuri/',
        '/construction-hub/'               => '/iezukuri/',
        '/construction-hub/concept'        => '/iezukuri/concept/',
        '/construction-hub/concept/'       => '/iezukuri/concept/',
        '/construction-hub/design-policy'  => '/iezukuri/design-policy/',
        '/construction-hub/design-policy/' => '/iezukuri/design-policy/',
        '/construction-hub/nasu-house'     => '/iezukuri/nasu-house/',
        '/construction-hub/nasu-house/'    => '/iezukuri/nasu-house/',
        '/construction-hub/design-office'  => '/iezukuri/design-office/',
        '/construction-hub/design-office/' => '/iezukuri/design-office/',
        '/construction-hub/company'        => '/iezukuri/company/',
        '/construction-hub/company/'       => '/iezukuri/company/',
        '/construction-hub/contact'        => '/iezukuri/contact/',
        '/construction-hub/contact/'       => '/iezukuri/contact/',
    );
...
/**
 * CONSTRUCTION HUB TO IEZUKURI REDIRECT END
 */

/**
 * =========================================================
 * IEZUKURI LOADER START
 *
 * 家づくり専用読み込み。
 * /iezukuri/ とサブページだけで CSS/JS を読む。
 * =========================================================
 */
// IEZUKURI EMPTY STOP: require_once get_template_directory() . '/hub/pages/hub/pages/iezukuri/inc/loader.php.DISABLED.DISABLED';
/**
 * IEZUKURI LOADER END
 */
...
/**
 * B2C CTA / contact form assets final.
 *
 * - B2C固定ページで contact form の元CSS/JSを読む
 * - CTA swiper .mnpk-cta-swiper を初期化する
 */
...
/* IEZUKURI CANONICAL LOADER START */
