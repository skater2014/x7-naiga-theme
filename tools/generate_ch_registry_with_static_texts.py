...
REPORT = Path("docs/ch-registry-with-static-texts-summary.txt")
...
def extract_static_fields(src, page_key):
    fields = {}
    idx = 1
    seen = set()
    ...
    for m in re.finditer(r"""(['"])(.*?[\u3040-\u30ff\u4e00-\u9fff].*?)\1""", src, flags=re.S):
        text = clean_text(m.group(2))
        if not keep_text(text):
            continue
        if text.startswith(("_ch_", "ch-", "hub-")) or ".php" in text:
            continue
        if text in seen:
            continue
        seen.add(text)

        key = f"_ch_{page_key}_static_{idx:03d}_php"
        fields[key] = {
            "key": key,
            "label": f"php / {text[:28]}",
...
lines = []
lines.append("<?php")
lines.append("/** Auto generated: /iezukuri ch registry with static texts. */")
...
for page_key, file_path in TARGETS.items():
    ...
    static_fields = extract_static_fields(src, page_key)

    for k, v in static_fields.items():
        if k not in fields:
            fields[k] = v

    summary.append(f"{page_key} / blocks={len(blocks)} / fields={len(fields)} / static={len(static_fields)} / {file_path}")
